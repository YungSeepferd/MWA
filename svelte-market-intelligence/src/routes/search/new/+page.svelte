<script>
  import { onMount } from 'svelte';
  import { enhance } from '$app/forms';
  import { createEventDispatcher } from 'svelte';

  export let data;

  let formData = {
    name: '',
    min_price: 500,
    max_price: 2000,
    min_rooms: 1,
    districts: [],
    move_in_date: '',
    amenities: [],
    providers: ['immoscout', 'wg_gesucht'],
    schedule_type: 'hourly',
    custom_interval: 60
  };

  let selectedDistricts = new Set();
  let editorMode = 'visual';
  let isLoading = false;
  let previewData = null;
  let showPreview = false;

  const dispatch = createEventDispatcher();

  // Munich districts data
  const munichDistricts = [
    { id: 1, name: 'Altstadt-Lehel', code: '01' },
    { id: 2, name: 'Ludwigsvorstadt-Isarvorstadt', code: '02' },
    { id: 3, name: 'Maxvorstadt', code: '03' },
    { id: 4, name: 'Schwabing-West', code: '04' },
    { id: 5, name: 'Au-Haidhausen', code: '05' },
    { id: 6, name: 'Schwabing-Freimann', code: '06' },
    { id: 7, name: 'Neuhausen-Nymphenburg', code: '07' },
    { id: 8, name: 'Moosach', code: '08' },
    { id: 9, name: 'Milbertshofen-Am Hart', code: '09' },
    { id: 10, name: 'Schwanthalerhöhe', code: '10' },
    { id: 11, name: 'Laim', code: '11' },
    { id: 12, name: 'Thalkirchen-Obersendling-Forstenried', code: '12' },
    { id: 13, name: 'Hadern', code: '13' },
    { id: 14, name: 'Pasing-Obermenzing', code: '14' },
    { id: 15, name: 'Aubing-Lochhausen-Langwied', code: '15' },
    { id: 16, name: 'Ramersdorf-Perlach', code: '16' },
    { id: 17, name: 'Trudering-Riem', code: '17' },
    { id: 18, name: 'Berg am Laim', code: '18' },
    { id: 19, name: 'Bogenhausen', code: '19' },
    { id: 20, name: 'Untergiesing-Harlaching', code: '20' },
    { id: 21, name: 'Thalkirchen-Obersendling-Forstenried-Fürstenried', code: '21' },
    { id: 22, name: 'Laim', code: '22' },
    { id: 23, name: 'Lohhof', code: '23' },
    { id: 24, name: 'Taufkirchen', code: '24' },
    { id: 25, name: 'Unterhaching', code: '25' }
  ];

  // Group amenities by category
  const groupedAmenities = data.amenities.reduce((groups, amenity) => {
    const category = amenity.category || 'other';
    if (!groups[category]) groups[category] = [];
    groups[category].push(amenity);
    return groups;
  }, {});

  // Toggle district selection
  const toggleDistrict = (districtId) => {
    if (selectedDistricts.has(districtId)) {
      selectedDistricts.delete(districtId);
    } else {
      selectedDistricts.add(districtId);
    }
    formData.districts = Array.from(selectedDistricts);
  };

  // Toggle provider selection
  const toggleProvider = (providerId) => {
    const providers = new Set(formData.providers);
    if (providers.has(providerId)) {
      providers.delete(providerId);
    } else {
      providers.add(providerId);
    }
    formData.providers = Array.from(providers);
  };

  // Toggle amenity selection
  const toggleAmenity = (amenityId) => {
    const amenities = new Set(formData.amenities);
    if (amenities.has(amenityId)) {
      amenities.delete(amenityId);
    } else {
      amenities.add(amenityId);
    }
    formData.amenities = Array.from(amenities);
  };

  // Preview search
  const previewSearch = async () => {
    try {
      isLoading = true;
      const response = await fetch('/api/v1/search/preview', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify(formData)
      });

      if (response.ok) {
        previewData = await response.json();
        showPreview = true;
      }
    } catch (error) {
      console.error('Error previewing search:', error);
    } finally {
      isLoading = false;
    }
  };

  // Save search
  const saveSearch = async () => {
    try {
      isLoading = true;
      const response = await fetch('/api/v1/search/save', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify(formData)
      });

      if (response.ok) {
        dispatch('navigate', { url: '/search' });
      }
    } catch (error) {
      console.error('Error saving search:', error);
    } finally {
      isLoading = false;
    }
  };

  // Start search immediately
  const startSearch = async () => {
    await previewSearch();
    // In a real implementation, this would start the search after preview
  };
</script>

<div class="search-creation">
  <div class="container mx-auto px-4 py-6 max-w-6xl">
    <!-- Header -->
    <div class="mb-8">
      <h1 class="text-3xl font-bold text-gray-900 mb-2">Create New Search</h1>
      <p class="text-gray-600">Configure your apartment search criteria and schedule</p>
    </div>

    <!-- Editor Mode Toggle -->
    <div class="flex justify-center mb-6">
      <div class="bg-gray-100 rounded-lg p-1 flex">
        <button
          class:active={editorMode === 'visual'}
          class="px-4 py-2 rounded-md transition-colors"
          on:click={() => editorMode = 'visual'}
        >
          Visual Editor
        </button>
        <button
          class:active={editorMode === 'advanced'}
          class="px-4 py-2 rounded-md transition-colors"
          on:click={() => editorMode = 'advanced'}
        >
          Advanced
        </button>
      </div>
    </div>

    <form method="POST" action="?/create" use:enhance class="space-y-8">
      <!-- Basic Information -->
      <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
        <h2 class="text-xl font-semibold text-gray-900 mb-4">Basic Information</h2>
        
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div>
            <label for="search-name" class="block text-sm font-medium text-gray-700 mb-2">
              Search Name
            </label>
            <input
              id="search-name"
              type="text"
              bind:value={formData.name}
              class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-primary-500"
              placeholder="My Apartment Search"
              required
            />
          </div>
        </div>
      </div>

      <!-- Price Range -->
      <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
        <h2 class="text-xl font-semibold text-gray-900 mb-4">Price Range</h2>
        
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div>
            <label for="min-price" class="block text-sm font-medium text-gray-700 mb-2">
              Minimum Price (€)
            </label>
            <input
              id="min-price"
              type="number"
              bind:value={formData.min_price}
              min="0"
              max="10000"
              class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-primary-500"
            />
          </div>
          
          <div>
            <label for="max-price" class="block text-sm font-medium text-gray-700 mb-2">
              Maximum Price (€)
            </label>
            <input
              id="max-price"
              type="number"
              bind:value={formData.max_price}
              min="0"
              max="10000"
              class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-primary-500"
            />
          </div>
        </div>
        
        <div class="mt-4">
          <div class="text-sm text-gray-600">
            Price Range: €{formData.min_price} - €{formData.max_price}
          </div>
        </div>
      </div>

      <!-- Room Selection -->
      <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
        <h2 class="text-xl font-semibold text-gray-900 mb-4">Rooms</h2>
        
        <div class="flex flex-wrap gap-4">
          {#each [1, 2, 3, 4, 5] as rooms}
            <label class="flex items-center">
              <input
                type="radio"
                name="rooms"
                value={rooms}
                bind:group={formData.min_rooms}
                class="mr-2"
              />
              <span class="text-sm text-gray-700">{rooms}+ rooms</span>
            </label>
          {/each}
        </div>
      </div>

      <!-- District Selection -->
      <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
        <h2 class="text-xl font-semibold text-gray-900 mb-4">Munich Districts</h2>
        <p class="text-sm text-gray-600 mb-4">Select the districts you want to search in</p>
        
        <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-3">
          {#each munichDistricts as district}
            <button
              type="button"
              class:selected={selectedDistricts.has(district.id)}
              class="p-3 border border-gray-300 rounded-lg text-center hover:bg-gray-50 transition-colors"
              on:click={() => toggleDistrict(district.id)}
            >
              <div class="text-sm font-medium text-gray-900">{district.code}</div>
              <div class="text-xs text-gray-600 truncate">{district.name}</div>
            </button>
          {/each}
        </div>
        
        <div class="mt-4 text-sm text-gray-600">
          Selected: {selectedDistricts.size} districts
        </div>
      </div>

      <!-- Move-in Date -->
      <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
        <h2 class="text-xl font-semibold text-gray-900 mb-4">Move-in Date</h2>
        
        <input
          type="date"
          bind:value={formData.move_in_date}
          class="px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-primary-500"
        />
      </div>

      <!-- Providers -->
      <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
        <h2 class="text-xl font-semibold text-gray-900 mb-4">Providers</h2>
        <p class="text-sm text-gray-600 mb-4">Select the platforms to search on</p>
        
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
          {#each data.providers as provider}
            <label class="flex items-center p-3 border border-gray-300 rounded-lg hover:bg-gray-50 cursor-pointer">
              <input
                type="checkbox"
                checked={formData.providers.includes(provider.id)}
                on:change={() => toggleProvider(provider.id)}
                class="mr-3"
              />
              <span class="text-sm font-medium text-gray-700">{provider.name}</span>
              {#if !provider.enabled}
                <span class="ml-2 px-2 py-1 text-xs bg-yellow-100 text-yellow-800 rounded">Coming Soon</span>
              {/if}
            </label>
          {/each}
        </div>
      </div>

      <!-- Amenities -->
      <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
        <h2 class="text-xl font-semibold text-gray-900 mb-4">Amenities</h2>
        
        {#each Object.entries(groupedAmenities) as [category, amenities]}
          <div class="mb-6">
            <h3 class="text-lg font-medium text-gray-900 mb-3 capitalize">{category}</h3>
            <div class="grid grid-cols-2 md:grid-cols-4 gap-3">
              {#each amenities as amenity}
                <label class="flex items-center">
                  <input
                    type="checkbox"
                    checked={formData.amenities.includes(amenity.id)}
                    on:change={() => toggleAmenity(amenity.id)}
                    class="mr-2"
                  />
                  <span class="text-sm text-gray-700">{amenity.name}</span>
                </label>
              {/each}
            </div>
          </div>
        {/each}
      </div>

      <!-- Schedule -->
      <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
        <h2 class="text-xl font-semibold text-gray-900 mb-4">Schedule</h2>
        
        <div class="space-y-4">
          <div class="flex flex-wrap gap-4">
            <label class="flex items-center">
              <input
                type="radio"
                name="schedule"
                value="hourly"
                bind:group={formData.schedule_type}
                class="mr-2"
              />
              <span class="text-sm text-gray-700">Hourly</span>
            </label>
            <label class="flex items-center">
              <input
                type="radio"
                name="schedule"
                value="daily"
                bind:group={formData.schedule_type}
                class="mr-2"
              />
              <span class="text-sm text-gray-700">Daily</span>
            </label>
            <label class="flex items-center">
              <input
                type="radio"
                name="schedule"
                value="custom"
                bind:group={formData.schedule_type}
                class="mr-2"
              />
              <span class="text-sm text-gray-700">Custom</span>
            </label>
          </div>
          
          {#if formData.schedule_type === 'custom'}
            <div class="flex items-center space-x-2">
              <label for="custom-interval" class="text-sm text-gray-700">
                Interval (minutes):
              </label>
              <input
                id="custom-interval"
                type="number"
                bind:value={formData.custom_interval}
                min="5"
                max="1440"
                class="w-20 px-2 py-1 border border-gray-300 rounded"
              />
            </div>
          {/if}
        </div>
      </div>

      <!-- Action Buttons -->
      <div class="flex justify-end space-x-4 pt-6">
        <button
          type="button"
          class="px-6 py-2 border border-gray-300 rounded-lg hover:bg-gray-50 transition-colors"
          on:click={() => dispatch('navigate', { url: '/search' })}
        >
          Cancel
        </button>
        <button
          type="button"
          class="px-6 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition-colors"
          on:click={previewSearch}
          disabled={isLoading}
        >
          {isLoading ? 'Previewing...' : 'Preview Search'}
        </button>
        <button
          type="button"
          class="px-6 py-2 bg-primary-600 text-white rounded-lg hover:bg-primary-700 transition-colors"
          on:click={saveSearch}
          disabled={isLoading}
        >
          {isLoading ? 'Saving...' : 'Save Search'}
        </button>
        <button
          type="button"
          class="px-6 py-2 bg-green-600 text-white rounded-lg hover:bg-green-700 transition-colors"
          on:click={startSearch}
          disabled={isLoading}
        >
          {isLoading ? 'Starting...' : 'Start Search'}
        </button>
      </div>
    </form>
  </div>
</div>

<style>
  button.selected {
    background-color: var(--primary-600);
    color: white;
    border-color: var(--primary-600);
  }

  button.active {
    background-color: var(--primary-600);
    color: white;
  }
</style>